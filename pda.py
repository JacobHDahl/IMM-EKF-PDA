...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def update(
       ...
       # remove the not gated measurements from consideration
        ...
        beta = self.association_probabilities(Zgated, filter_state, sensor_state=sensor_state)

        # find the mixture components
        filter_state_update_mixture_components = self.conditional_update(Zgated, filter_state, sensor_state=sensor_state)
